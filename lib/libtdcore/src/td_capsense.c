...
/** ADC channel to use for CAPSENSE */
#define ACMP_CAPSENSE                           ACMP0

/** Clock source to use for CAPSENSE */
#define ACMP_CAPSENSE_CLKEN                     CMU_HFPERCLKEN0_ACMP0

/** PRS source to use for CAPSENSE */
#define PRS_CH_CTRL_SOURCESEL_ACMP_CAPSENSE     PRS_CH_CTRL_SOURCESEL_ACMP0

/** PRS signal to use for CAPSENSE */
#define PRS_CH_CTRL_SIGSEL_ACMPOUT_CAMPSENSE    PRS_CH_CTRL_SIGSEL_ACMP0OUT

/** CAPSENSE activation threshold in 1/1000 unit */
#define DELTA_CHANGE_THRESHOLD 50

/** CAPSENSE history depth (max push time) */
#define CAPSENSE_HISTORY_DEPTH 30
...
/** CAPSENSE value history */
static uint32_t CapSenseValueHistory[CAPSENSE_HISTORY_DEPTH];
...
static bool CAPSENSE_getPressed(void)
{
	...
	DEBUG_PRINTF("D%d\r\n",delta);
	if (delta <= -DELTA_CHANGE_THRESHOLD) {
		if ((MaxValueVoltage -
...
static void CAPSENSE_Measure(void)
{
	...
	ACMP_CapsenseChannelSet(ACMP_CAPSENSE, (ACMP_Channel_TypeDef) 0);
	...
	CapSenseValueHistory[CapSenseValueHistoryIndex++] = CapSenseValue;
	CapSenseValueHistoryIndex %= CAPSENSE_HISTORY_DEPTH;
	previous_max = MaxValue;
	MaxValue = 0;
	for (i = 0; i < CAPSENSE_HISTORY_DEPTH; i++) {
		if (CapSenseValueHistory[i] > MaxValue) {
			MaxValue = CapSenseValueHistory[i];
		}
	}
...
void TD_CAPSENSE_Init(void)
{
	...
	memset((uint8_t *) CapSenseValueHistory, 0, CAPSENSE_HISTORY_DEPTH);
	MaxValue = 0xFFFFFFFF;
	MaxValueVoltage = 0;
	CAPSENSE_Measure();
}
