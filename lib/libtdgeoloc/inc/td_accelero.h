...
/** @} */
	...
	typedef struct {
		uint8_t axis; ///< Axis to monitor
		void (*user_callback)(TD_ACCELERO_Data_t data[32], uint8_t count,
			bool overrun);///< Data monitoring callback function
	...
	void TD_ACCELERO_MonitorData(bool enable, bool low_power,
		...
		void (*callback)(TD_ACCELERO_Data_t data[32], uint8_t count,
			bool overrun));
